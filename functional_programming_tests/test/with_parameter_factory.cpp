...
struct PathGenerator : public rclcpp::Node {
  /**
   * @brief Parameters for the path generator
   */
  struct Parameters {
    /**
  ...
  /**
   * @brief Generates a path from a start to a goal position
   * @param params Parameters for the path generator
   */
  explicit PathGenerator(Parameters const& params)
      : Node("path_generator"),
        robot_size_{params.robot_size},
        is_occupied_{robot_size_} {
 ...
 private:
  ...
  int robot_size_;
  CollisionChecker<unsigned char> is_occupied_;
...
/**
 * @brief Factory function for fetching PathGenerator::Parameters from the
 *        ROS2 parameter server
 * @param node to fetch parameters from
 * @return PathGenerator::Parameters
 * @note This is not used in the unit tests but would be used in main.
 *       A version that didn't have sensible defaults would return a
 *       std::optional<PathGenerator::Parameters> instead.
 */
PathGenerator::Parameters get_parameters(std::shared_ptr<rclcpp::Node> node) {
  PathGenerator::Parameters parameters;
  parameters.robot_size = node->declare_parameter<int>("robot_size", 1);
  return parameters;
}
...
TEST_F(TaskPlanningFixture, same_start_and_goal) {
  ...
  auto parameters = PathGenerator::Parameters{};
  parameters.robot_size = 1;
  auto const pg = std::make_shared<PathGenerator>(parameters);
...
TEST_F(TaskPlanningFixture, medium_footprint_with_path) {
  ...
  // GIVEN a PathGenerator node with a medium robot footprint
  auto parameters = PathGenerator::Parameters{};
  parameters.robot_size = 2;
  auto const pg = std::make_shared<PathGenerator>(parameters);
